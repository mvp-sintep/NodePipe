...
const server = http.createServer((request, response) => { // Функция http сервера

  var url = new URL(request.url, "http://127.0.0.1:1337/");

  if (url.pathname == '/') { // Если запрошен корневой элемент
    ...
    response.write('<body>'); // Начало тела
    response.write(
      '<table class="plcs">' +
    ...
    response.write('</table>'); // Закончили с таблицей контроллеров
    ...
    if (url.searchParams.get('id') != undefined) {
      TAG.find((arg) => {
        if (arg.id == url.searchParams.get('id')) {
          var tmp, val, cnt; // Временные переменные
          if (url.searchParams.get('count') === null || url.searchParams.get('count') < 1000) cnt = 5000; else cnt = url.searchParams.get('count');
          switch (arg.type) { // Выбираем значение и таблицу по типу тега
            case 'SINT':
            case 'INT':
            case 'DINT': val = "value"; tmp = "i"; break; // Для всех целочисленных типов таблица i
            case 'REAL': val = "round( value * 100 ) / 100"; tmp = "r"; break; // Для типа real - r
            case 'BOOL': val = "value"; tmp = "b"; break; // Для типа boolean - b
          }
          pool
            ...
            .then((result) => {
              ...
              pool.query({ text: "select dt, " + val + " from public." + tmp + " order by dt limit " + cnt + " offset ((select count (*) from public." + tmp + " ) - " + cnt + ");", rowMode: 'array' }, (error, results) => {
                if (error) { throw error }
                response.writeHead(200, { 'Content-Type': 'application/json;' });
                response.write('Дата и время,' + arg.user);
    ...
    response.end('<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>[' + request.url + '] not found</body></html>'); // И телом пустого документа
  }
});
